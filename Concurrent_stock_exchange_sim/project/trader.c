...
void client_communication_trader(command_client *cmd);
...
int main(int argc, char *argv[]) {
    ...
    client_communication_trader(&cmd);
...
void client_communication_trader(command_client *cmd) {
    ...
    if(num_read > 0) {
        ...
        while(1) {
            ...
            if(!done && FD_ISSET(STDIN_FILENO, &read_fds)) {
                ...
                if(!done) {
                    special_write(sock_fd, message, strlen(message));
                    printf("[TRADER %s] SENT %s", cmd->username, message);
                }
